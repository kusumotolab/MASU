...
public class CFGExpressionNode extends CFGNormalNode<ExpressionInfo> {
    ...
    ExpressionInfo getDissolvingTarget() {
        ...
        return (ExpressionInfo) expression.copy();
    }

    @Override
    ExpressionInfo makeNewElement(final LocalSpaceInfo ownerSpace, final int fromLine,
        ...
        if (expression instanceof BinominalOperationInfo) {
            ...
            if (operator.equals(OPERATOR.ASSIGN)) {
                final BinominalOperationInfo newExpression = new BinominalOperationInfo(operator,
                        firstOperand, requiredExpressions[0], outerCallableUnit, fromLine,
