...
public class CFGForeachControlNode extends CFGControlNode {
    ...
    public CFG dissolve(final ICFGNodeFactory nodeFactory) {
        ...
        // 分解前の文から必要な情報を取得
        final ForeachConditionInfo condition = (ForeachConditionInfo) this.getCore();
        ...
        final ExpressionInfo newIteratorExpression = LocalVariableUsageInfo.getInstance(
                newStatement.getDeclaredLocalVariable(), true, false, ownerForeachBlock
                        .getOuterCallableUnit(), fromLine, fromColumn, toLine, toColumn);
        final ForeachConditionInfo newCondition = this.makeNewElement(ownerForeachBlock,
    ...
    ExpressionInfo getDissolvingTarget() {
        final ForeachConditionInfo condition = (ForeachConditionInfo) this.getCore();
        return (ExpressionInfo) condition.getIteratorExpression().copy();
    }
     ...
     */
    @Override
    ForeachConditionInfo makeNewElement(final LocalSpaceInfo ownerSpace, final int fromLine,
        ...
        final ForeachConditionInfo originalCondition = (ForeachConditionInfo) this.getCore();
        ...
        return new ForeachConditionInfo(ownerMethod, fromLine, fromColumn, toLine, toColumn,
                iteratorVariable, requiredExpressions[0]);
    }
