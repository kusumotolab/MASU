...
public class IntraProceduralCFG extends CFG {
     ...
     */
    public IntraProceduralCFG(final CallableUnitInfo unit, final ICFGNodeFactory nodeFactory,
            final boolean optimize, final boolean dissolve) {
        ...
        final SequentialStatementsCFG statementsCFG = new SequentialStatementsCFG(unit
                .getStatementsWithoutSubsequencialBlocks(), nodeFactory);
    ...
    private IntraProceduralCFG(final StatementInfo statement, final ICFGNodeFactory nodeFactory) {
        ...
        if (statement instanceof SingleStatementInfo) {
            ...
            // 例外に関する処理
            for (final ReferenceTypeInfo thrownException : statement.getThrownExceptions()) {
        ...
        // if文の場合
            ...
            if (ifBlock.hasElseBlock()) {
                final SequentialStatementsCFG statementsCFG = new SequentialStatementsCFG(ifBlock
                        .getSequentElseBlock().getStatementsWithoutSubsequencialBlocks(),
                        nodeFactory);
                this.nodes.addAll(statementsCFG.nodes);
        ...
        // synchorized文の場合
            ...
            final SequentialStatementsCFG statementsCFG = new SequentialStatementsCFG(
                    synchronizedBlock.getStatementsWithoutSubsequencialBlocks(), nodeFactory);
            this.enterNode = statementsCFG.getEnterNode();
            this.exitNodes.addAll(statementsCFG.getExitNodes());
            this.nodes.addAll(statementsCFG.nodes);
        }
        ...
        for (final CFGNode<? extends ExecutableElementInfo> node : preAllNodes) {
            ...
            if (null != dissolvedCFG) {
                ...
                if (this.enterNode == node) {
                    this.enterNode = CFGNode.getHeadmostNode(dissolvedCFG.getEnterNode());
                }

                // disolvedNodesに登録
                final ExecutableElementInfo element = node.getCore();
                this.nodeFactory.addDissolvedNodes(element, dissolvedCFG.getAllNodes());
