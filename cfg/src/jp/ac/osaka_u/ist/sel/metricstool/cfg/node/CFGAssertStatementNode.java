...
public class CFGAssertStatementNode extends CFGStatementNode<AssertStatementInfo> {
    ...
    ExpressionInfo getDissolvingTarget() {
        final AssertStatementInfo statement = this.getCore();
        return (ExpressionInfo) statement.getAssertedExpression().copy();
    }

    @Override
    AssertStatementInfo makeNewElement(final LocalSpaceInfo ownerSpace, final int fromLine,
        ...
        final AssertStatementInfo newStatement = new AssertStatementInfo(ownerSpace,
                requiredExpressions[0], messageExpression, fromLine, fromColumn, toLine, toColumn);
        return newStatement;
