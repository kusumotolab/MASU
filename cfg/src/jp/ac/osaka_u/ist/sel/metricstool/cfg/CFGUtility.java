...
public class CFGUtility {
    ...
    static public boolean stateChange(final MethodInfo method) {
        ...
        for (final CallInfo<? extends CallableUnitInfo> call : method.getCalls()) {
     ...
     * @return
     */
    static public boolean stateChange(final CallableUnitInfo method, final int index) {

        // method がExternalで引数がない場合は，解析をあきらめる
        if ((method instanceof ExternalMethodInfo) || (method instanceof ExternalConstructorInfo)) {
        ...
        if (index < method.getParameters().size()) {
            ...
            parameter = method.getParameters().get(method.getParameters().size() - 1);
        }
                        ...
                        if (parameter.equals(usedVariable)) {
                            ...
                            if (callee instanceof MethodInfo) {
                                for (final MethodInfo overrider : ((MethodInfo) callee)
                                        .getOverriders()) {
                                    if (stateChange(overrider, i, checkedMethods)) {
                                        return true;
                                    }
                                }
                            }
    ...
    static private boolean stateChange(final CallableUnitInfo method, final int index,
            final Set<CallableUnitInfo> checkedMethods) {
