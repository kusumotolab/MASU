...
public final class Configuration {
     ...
     * no: 変数名をそのまま使う
     * type: 変数名を型名に正規化する．
     * all: 全ての変数を同一字句に正規化する．
     */
    private VARIABLE_NORMALIZATION pv;

    /**
     * (メソッドまたはコンストラクタ)呼び出しの正規化レベルを指定するためのオプション
     * no: 呼び出し名はそのまま，引数情報も用いる
     * type_with_arg: 呼び出し名を返り値の型名に変換する，引数情報も用いる．
     * type_without_arg: 呼び出し名を返り値の型名に変換する，引数情報は用いない．
     * all: 全ての呼び出しを同一字句に正規化する．引数情報は用いない
     */
