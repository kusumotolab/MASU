...
public class IntraProceduralSlicer {
    ...
    public Set<PDGNode<?>> extratForwardSlice(
    ...
    private void buildForwardSlice(
            final Set<PDGNode<?>> buildingSlice,
            final PDGNode<?> newNode) {
    ...
    public Set<PDGNode<?>> extratBackwardSlice(
            final VariableInfo<? extends UnitInfo> variable) {
        final Set<PDGNode<?>> slice = new HashSet<PDGNode<?>>();
    ...
    private void buildBackwardSlice(
            final Set<PDGNode<?>> buildingSlice,
            final PDGNode<?> newNode) {
    ...
    public Set<PDGNode<?>> extratMetricSlice(
            final VariableInfo<? extends UnitInfo> variable) {
        final Set<PDGNode<?>> slice = new HashSet<PDGNode<?>>();
    ...
    private PDGNode<?> getStartNode(final VariableInfo<? extends UnitInfo> variable) {
        if (variable instanceof LocalVariableInfo) {
            return this.pdg.getPDGNode(((LocalVariableInfo) variable).getDeclarationStatement());
        } else if (variable instanceof ParameterInfo) {
            return this.pdg.getParamerNode((ParameterInfo) variable);
        }
