...
public class InterProceduralPDG extends IntraProceduralPDG {
     ...
     */
    public InterProceduralPDG(final CallableUnitInfo unit, final IPDGNodeFactory pdgNodeFactory,
        ...
        super(unit, pdgNodeFactory, cfgNodeFactory, buildDataDependency, buildControlDependencey,
    ...
    public InterProceduralPDG(final CallableUnitInfo unit, final boolean buildDataDependency,
            final boolean buildControlDependencey, final boolean buildExecutionDependency) {
        super(unit, buildDataDependency, buildControlDependencey, buildExecutionDependency);
        PDG_MAP.put(unit, this);
    }
    ...
    private void addCallEdges(final SingleStatementInfo statement) {
        for (final CallInfo<?> call : statement.getCalls()) {
            ...
            final PDG calleePDG = PDG_MAP.get(callee);
            assert null != calleePDG : "Illegal State!";

            final PDGNode<?> fromNode = this.nodeFactory.getNode(statement);
            for (final PDGNode<?> toNode : calleePDG.getEnterNodes()) {
                final PDGCallDependenceEdge edge = new PDGCallDependenceEdge(fromNode, toNode, call);
            }
        }
    }
